...
from quality.views import EvalRunAPIView, QualityRunStatusView, QualityHumanRatingUpdateView
from chat.views import ChatPageView
from users.views import HomeView, CustomerRegisterView, CustomLoginView, CustomLogoutView, CustomerProfileView
...
urlpatterns += [
    ...
    # START: Quality Run (GET = Seite, POST = Run starten)
    path(
        "staff/quality/",
        StaffQualityView.as_view(),
        name="staff-quality"
    ),

    path("staff/quality/questions/", StaffQualityQuestionsView.as_view(), name="quality-questions"),

    # STATUS: Polling während Run
    path(
        "staff/quality/status/<int:run_id>/",
        QualityRunStatusView.as_view(),
        name="quality-run-status"
    ),
]
